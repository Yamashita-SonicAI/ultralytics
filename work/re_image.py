...
def overlay_mask_whiten_fast(bgr, mask01, alpha=0.35):
    # findContours は重いのでやらない（必要なら戻してOK）
        ...
        while True:
            ...
            if not vp_ready:
                ...
                if key == ord("s"):
                    ...
                    # ★ここが肝：VPEを「一回だけ」セットする初期化predict
                    # sourceは参照画像でOK（この1回でvpe->set_classesが走る）
            ...
            # -----------------------------
            # ここから高速推論（重要：refer_image/visual_prompts を渡さない）
            # -----------------------------
            ...
            draw_overlay(disp, [
                ...
                "'r' : re-select ROI (reset VPE)   'q' : quit",
            ])
            cv2.imshow(win, disp)
