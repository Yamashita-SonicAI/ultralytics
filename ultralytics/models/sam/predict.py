...
class SAM3SemanticPredictor(SAM3Predictor):
    ...
    def _inference_features(
        ...
        geometric_prompt = self._get_dummy_prompt(nc) if visual_prompt_embed is not None else None
        if bboxes is not None:
        ...
        # Match visual prompt batch size to the number of text prompts.
        if visual_prompt_embed is not None:
